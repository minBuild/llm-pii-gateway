...
- 2026-07-24 인젝션 ML 분류기 '맛보기'(R2 잔여, 실험): `tests/util/injection_ml_eval.py` — TF-IDF char n-gram + LogisticRegression, **합성 66건 훈련 / 독립셋 테스트**(injection_eval, 훈련 미사용)로 과적합 정직 측정. 결과: 표준 held-out 은 규칙과 동률(0.93/0.93, 이득 0)이나 **키워드 없는 의역 6건에서 규칙 0 → ML 5** 로 상보적 값어치 확인(정규식이 구조상 눈먼 축). 단 margin 얕음(명백 인젝션 P≈0.58)·합성 데이터 → **요청 경로 미배포**(약한 모델을 넣지 않는 규율 유지). 결론: 접근은 검증됐고 진짜 다음 단계는 실데이터 확보. sklearn 은 optional `[ml]` extra. LLM-judge 는 순환·비용으로 비채택.
- 2026-07-24 벤치마크(부하/지연 + Python vs JVM): `bench/bench_py.py`(stdlib) + `bench/Bench.java`(JDK25 단일파일, 빌드도구 불필요) 동일 워크로드. 결과(18코어): **필터 오버헤드 최악 heavy p99 ~52µs**(LLM 호출 대비 무시). 단일스레드 per-op Java 1.3~1.6× 빠름(정규식엔진+JIT). **처리량 핵심**: Python GIL 로 1↔18스레드 모두 ~20.7k ops/s 평평(확장 불가, 스케일하려면 multiprocessing), JVM 플랫폼스레드 18코어 ~362k ops/s(~17×). Java 는 핫패스 대표 재구현(사양 100% 패리티 아님)·CPU 마이크로벤치(가상스레드 이점은 여기 아닌 프록시 I/O 동시성). 단계분해: detect_l1 25.6µs·injection 18µs 가 비용 대부분, normalize 0.9µs. 결과·한계는 `bench/README.md`. "Python 먼저 → 처리량 중요해지면 JVM" 방향을 수치로 뒷받침.
