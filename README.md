...
- [x] **Phase 1** — kpii 코어: L1 정규식·체크섬 탐지 + 가역 마스킹 + 스트리밍 복원. 오프라인 단위테스트 51건 통과.
- [~] **Phase 0** — 스캐폴딩/LiteLLM 골격: 파일 작성 완료. `make up` 헬스체크는 로컬(키·이미지 필요)에서 실행.
- [ ] **Phase 2** — 게이트웨이 요청 방향 마스킹/차단 (guardrail 어댑터 + fake upstream 통합테스트)
...
| `litellm/` | LiteLLM 설정 + 커스텀 guardrail 어댑터(Phase 2) |
