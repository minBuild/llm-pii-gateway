...
- [~] **Phase 2** — 요청 방향 마스킹/차단: guardrail 어댑터 + 요청 스캔 코어 + fake upstream. 오프라인 단위테스트 통과(67건, litellm/fastapi 검증 포함). 프록시 E2E 통합테스트는 로컬 도커(`make up-test && make test-integration`).
...
make test-unit    # 67 passed (litellm/fastapi 있으면 guardrail·fake-upstream 테스트도 실행)
```
