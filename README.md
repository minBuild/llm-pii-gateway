...
- [x] **Phase 2** — 요청 방향 마스킹/차단: guardrail 어댑터 + 요청 스캔 코어. 오프라인 단위테스트 + **라이브 프록시 E2E 검증(8/8)**.
- [x] **Phase 3** — 응답 복원(비스트리밍·스트리밍): post-call 훅 + StreamRestorer. 오프라인 + **라이브 E2E 검증**. (mode는 `[pre_call, post_call]` 둘 다 필요)
- [~] **Phase 4** — Presidio 한국어 NER 사이드카(이름·주소 L2) + 장애 격리(degrade/block). 비동기 Presidio 클라이언트·L1+L2 병합·guardrail NER 경로 **오프라인 검증(단위 85건)**. 실제 Presidio E2E는 로컬(모델이 무거워 이 환경 미실행).
- [x] **Phase 5** — 감사 로그(JSONL)·Prometheus 메트릭·무유출 테스트(컴포넌트 90건 + 라이브 litellm 로그 grep). `docs/operations.md`·`docs/onboarding.md`.
